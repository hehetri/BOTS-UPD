...
def message_box(target, title, message):
    packet = PacketWrite()
    packet.add_header(bytearray([0x12, 0x2F]))
    packet.append_bytes(bytearray([0x01, 0x00]))
    packet.append_string(title, 15)
    packet.append_string(message, 98)
    try:
...
def chat(**_args):
    ...
    if message_body.startswith('@mission') or message_body.startswith('@missao'):
        ...
        if not lines:
            return chat_message(
                target=_args['client'],
                message='No quests found for this map.',
                color=2
            )
...
def get_lobby(**_args):
    ...
    if _args['client']['new']:
        ...
        login_count, rewards = LoginDiary.process_login(_args)
        reward_lines = "\n".join(["+ {0}".format(item) for item in rewards]) if rewards else "+ None"
        diary_message = (
            "Welcome to T-bot Online\n"
            "You've logged in {0} times this month so far!\n\n"
            "Today's login rewards:\n"
            "{1}"
        ).format(login_count, reward_lines)
