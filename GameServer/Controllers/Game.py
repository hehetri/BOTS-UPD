...
def load_finish(_args, room):
    ...
    # Re-broadcast wearing items to ensure in-game visuals are synced
    for slot_id, slot in room['slots'].items():
        wearing_update = PacketWrite(header=REPLY_ROOM_EXIT_SHOP)
        wearing_update.append_bytes(bytearray([0x01, 0x00]))
        wearing_update.append_integer(int(slot_id) - 1, 2, 'little')
        wearing_update.append_bytes(bytearray([0x00, 0x00]))
        wearing_update.append_integer(slot['client']['id'], 2, 'little')

        wearing_items = get_items(_args, slot['client']['character']['id'], 'wearing')
        for i in range(19):
            item = wearing_items['items'][i]
            wearing_update.append_integer(item['id'], 4, 'little')

        _args['connection_handler'].room_broadcast(room['id'], wearing_update.packet)

    if room['game_type'] == MODE_PLANET:
        for slot in room['slots'].values():
            quest_lines = Missions.format_missions_for_chat(
                _args,
...
def post_game_transaction(_args, room, status=None):
    ...
    for key, slot in room['slots'].items():
        ...
        if room['game_type'] == MODE_PLANET:
            slot_key = str(key)
            monster_kills = room['player_data'].get('monster_kills', {}).get(slot_key, 0)
            boss_killed = False
            if room['level'] in PLANET_BOX_MOBS and len(PLANET_BOX_MOBS[room['level']]) > 0:
                boss_id = PLANET_BOX_MOBS[room['level']][-1]
                boss_killed = boss_id in room['killed_mobs']

            mission_bonus_exp = Missions.process_planet_missions(
                _args,
                character,
                room['level'],
                slot['won'],
        ...
        if mission_bonus_exp > 0:
            addition_experience += mission_bonus_exp
...
def anti_hack_check(_args, room):
    """
    Anti-hack seguro para Planet Mode.
    NÃO suspende jogadores.
    Apenas registra inconsistências.
    """

    # Só aplica para Planet Mode
    if room['game_type'] != MODE_PLANET:
        return

    # Só valida quando o planeta TERMINAR
    if not room.get('finished', False):
        return

    # Segurança: dados obrigatórios
    if 'attack_points' not in room['player_data']:
        return

    if 'monster_kills' not in room['player_data']:
        return
    ...
    total_score = sum(room['player_data']['attack_points'].values())

    # Detecta mercenário corretamente
    has_merc = False
    for slot in room['slots'].values():
        if not slot:
            continue

        wearing_items = get_items(
            _args,
            slot['client']['character']['id'],
            'wearing'
        )

        for item in wearing_items.get('items', {}).values():
            if item.get('type', '').startswith('merc') and item.get('character_item_id'):
                has_merc = True
                break

    if minimum_score > total_score and not has_merc:
        print(
            "[ANTI_HACK WARNING] Low planet score",
            "room:", room.get('id'),
            "score:", total_score,
            "min:", minimum_score
        )
    ...
    total_mob_kills = sum(room['player_data']['monster_kills'].values())

    if min_mob_kills > total_mob_kills:
        print(
            "[ANTI_HACK WARNING] Low mob kills",
            "room:", room.get('id'),
            "kills:", total_mob_kills,
            "min:", min_mob_kills
        )
...
def anti_hack_fail(_args, room):
    """
    Anti-hack FAIL seguro.
    NÃO suspende jogadores.
    Apenas registra ocorrência.
    """
    ...
    for key, slot in room['slots'].items():
        ...
        user_id = slot['client']['account_id']
        print("[ANTI_HACK FLAG] user:", user_id)
...
def chat_command(**_args):
    ...
    if message.startswith('@'):
        ...
        if command.startswith('mission') or command.startswith('missao'):
            command_split = command.split()
            if len(command_split) < 2:
                map_id = room['level']
            else:
                try:
                    map_id = int(command_split[1])
                except ValueError:
                    return Lobby.chat_message(_args['client'], 'Invalid map id.', 2)
            ...
            lines = Missions.format_missions_for_chat(
                _args,
                _args['client']['character']['id'],
                map_id
            )
            ...
            if not lines:
                Lobby.chat_message(_args['client'], 'No quests found for this map.', 2)
            else:
                for line in lines:
                    Lobby.chat_message(_args['client'], line['message'], 2 if line['completed'] else 3)
