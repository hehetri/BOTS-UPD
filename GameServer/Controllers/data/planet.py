...
def _build_planet_missions():
    missions = {}
    for map_id, values in PLANET_MAP_TABLE.items():
        base_exp = values[0]
        completion_required = max(1, 2 + int(map_id / 12))
        min_kills_required = max(5, values[4])
        boss_id = None
        if map_id in PLANET_BOX_MOBS and len(PLANET_BOX_MOBS[map_id]) > 0:
        ...
        if map_id % 2 == 0:
            missions[map_id].append({
                ...
                'type': 'kills',
                'name': 'Eliminate {0} monsters on map {1}'.format(min_kills_required, map_id),
                'required': min_kills_required,
                'reward_exp': int(base_exp * 0.75),
                'target_id': None
            })
        else:
            missions[map_id].append({
                'key': 'complete_{0}'.format(map_id),
                'type': 'complete',
                'name': 'Finish map {0} {1}x'.format(map_id, completion_required),
                'required': completion_required,
                'reward_exp': int(base_exp * 0.5),
                'target_id': None
            })

        if boss_id is not None:
            missions[map_id].append({
                'key': 'boss_{0}'.format(map_id),
                'type': 'boss',
                'name': 'Defeat the map boss {0}'.format(map_id),
                'required': 1,
                'reward_exp': int(base_exp * 1.0),
                'target_id': boss_id
            })
...
PLANET_MISSIONS = _build_planet_missions()